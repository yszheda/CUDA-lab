...
int main(int argc, char *argv[])
{
	...
	// NOTE: threadNum and blockNum need to change for first-add technique
	cudaDeviceProp deviceProperties;
	cudaGetDeviceProperties(&deviceProperties, 0);
	int maxThreadsPerBlock = deviceProperties.maxThreadsPerBlock; // maxThreadsPerBlock = 1024 on K20X
	threadNum = (elemNum > maxThreadsPerBlock * 2)? maxThreadsPerBlock: (elemNum / 2);
	blockNum = (int) ceil((double) elemNum / (threadNum * 2));
	...
	threadNum = (elemNum > maxThreadsPerBlock * 2)? maxThreadsPerBlock: (elemNum / 2);
	blockNum = (int) ceil((double) blockNum / (threadNum * 2));
	while(blockNum >= 1) {
